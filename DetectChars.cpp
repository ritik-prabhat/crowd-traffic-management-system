...
bool loadKNNDataAndTrainKNN(void) {
    ...
    // read in training classifications ///////////////////////////////////////////////////
     ...
     FileStorage fsClassifications("/Users/shwetankmaheshwari/Downloads/OpenCV_3_License_Plate_Recognition_Cpp-master/classifications.xml",  FileStorage::READ);        // open the classifications file
    ...
    fsClassifications.release();                                            // close the classifications file
     ...
     FileStorage fsTrainingImages("/Users/shwetankmaheshwari/Downloads/OpenCV_3_License_Plate_Recognition_Cpp-master/images.xml",  FileStorage::READ);              // open the training images file
 ...
 vector<PossiblePlate> detectCharsInPlates( vector<PossiblePlate> &vectorOfPossiblePlates) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
...
#ifdef SHOW_STEPS
         imshow("5a", possiblePlate.imgPlate);
         imshow("5b", possiblePlate.imgGrayscale);
         imshow("5c", possiblePlate.imgThresh);
#endif    // SHOW_STEPS
        ...
        // threshold again to eliminate any gray areas
         threshold(possiblePlate.imgThresh, possiblePlate.imgThresh, 0.0, 255.0,  THRESH_BINARY |  THRESH_OTSU);

#ifdef SHOW_STEPS
         imshow("5d", possiblePlate.imgThresh);
#endif    // SHOW_STEPS
...
#ifdef SHOW_STEPS
        imgContours =  Mat(possiblePlate.imgThresh.size(), CV_8UC3, SCALAR_BLACK);
        contours.clear();
        ...
        for (auto &vectorOfMatchingChars : vectorOfVectorsOfMatchingCharsInPlate) {
            ...
            for (auto &matchingChar : vectorOfMatchingChars) {
             ...
             drawContours(imgContours, contours, -1,  Scalar((double)intRandomBlue, (double)intRandomGreen, (double)intRandomRed));
        }
         imshow("7", imgContours);
#endif    // SHOW_STEPS

        if (vectorOfVectorsOfMatchingCharsInPlate.size() == 0) {                // if no groups of matching chars were found in the plate
#ifdef SHOW_STEPS
             cout << "chars found in plate number " << intPlateCounter << " = (none), click on any image and press a key to continue . . ." <<  endl;
            intPlateCounter++;
             destroyWindow("8");
             destroyWindow("9");
             destroyWindow("10");
...
#ifdef SHOW_STEPS
         cout << "chars found in plate number " << intPlateCounter << " = " << possiblePlate.strChars << ", click on any image and press a key to continue . . ." <<  endl;
        intPlateCounter++;
         waitKey(0);
#endif    // SHOW_STEPS
...
#ifdef SHOW_STEPS
     cout <<  endl << "char detection complete, click on any image and press a key to continue . . ." <<  endl;
     waitKey(0);
#endif    // SHOW_STEPS
...
///////////////////////////////////////////////////////////////////////////////////////////////////
 vector<PossibleChar> findPossibleCharsInPlate( Mat &imgGrayscale,  Mat &imgThresh) {
    ...
    imgThreshCopy = imgThresh.clone();                // make a copy of the thresh image, this in necessary b/c findContours modifies the image
     ...
     findContours(imgThreshCopy, contours,  RETR_LIST,  CHAIN_APPROX_SIMPLE);        // find all contours in plate
...
// this is where we apply the actual char recognition
 string recognizeCharsInPlate( Mat &imgThresh,  vector<PossibleChar> &vectorOfMatchingChars) {
    ...
    // sort chars from left to right
     ...
     cvtColor(imgThresh, imgThreshColor,  COLOR_GRAY2BGR);       // make color version of threshold image so we can draw contours in color on it

    for (auto &currentChar : vectorOfMatchingChars) {           // for each char in plate
         rectangle(imgThreshColor, currentChar.boundingRect, SCALAR_GREEN, 2);       // draw green box around the char
